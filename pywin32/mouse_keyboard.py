...
def click(click_type, x_position=None, y_position=None, double_click=False, sleep=0.):
    ...
    if x_position is not None and y_position is not None:
        ...
        # win32api.ClipCursor((x_position-1, y_position-1, x_position+1, y_position+1))
...
def press(sleep=0, *args):
    ...
    for arg in args:
        if arg in VK_SHIFT_CODE:
            press_hold_release(sleep, "left_shift", VK_SHIFT_CODE[arg])
        else:
...
def press_hold_release(sleep=0, *args):
    """
    组合建按下与释放
    :param args:
    :return:
